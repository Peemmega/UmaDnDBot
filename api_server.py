...
async def api_tcg_create_room(payload: TcgPlayerPayload):
    room = tcg_room_manager.create_room(payload.user_id, payload.username, payload.avatar_url)
...
async def api_tcg_join_room(room_id: str, payload: TcgPlayerPayload):
    try:
        room = tcg_room_manager.join_room(room_id, payload.user_id, payload.username, payload.avatar_url)
...
async def websocket_tcg_room(websocket: WebSocket, room_id: str, user_id: str):
    ...
    try:
        while True:
            ...
            try:
                tcg_room_manager.apply_action(room_id, user_id, message)
                await tcg_room_manager.broadcast(room_id)
            except ValueError as exc:
