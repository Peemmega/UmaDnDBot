...
from utils.race.race_completion import complete_race
from utils.skill.skill_effect_registry import (
    SkillEffectContext,
    apply_registered_skill_effects,
)
...
def apply_lane_tactics_to_result(
    *,
    ...
    stamina_cost = calculate_roll_stamina_cost(
        game_player,
        path_effect,
    ...
    weather_stamina_cost = stamina_cost.weather_cost
    stamina_drain = stamina_cost.total
...
def apply_skill(channel_id: int, user_id: int, skill: dict):
    ...
    context = SkillEffectContext(
        ...
        apply_race_stat_changes=apply_race_stat_changes,
        format_player_reference=format_player_reference,
        increase_speed=incrase_speed_by_acceleration,
        apply_pending_lane_change_now=apply_pending_lane_change_now,
        can_force_rush_targets=can_force_rush_targets,
        use_rush=use_rush,
        execute_skill_core=execute_skill_core,
    )
    success, result = apply_registered_skill_effects(
        context, list(skill.get("effects", []))
    )
    if not success:
        return False, result
    if not result:
